// Package http provides the HTTP server and shared middleware for the API.
package http

import (
	"net/http"
	"strconv"
	"time"

	"github.com/go-chi/chi/v5"
	"github.com/go-chi/chi/v5/middleware"
)

// RouterOptions configures the shared middleware chain.
type RouterOptions struct {
	// CORSAllowedOrigins enables cross-origin access for these origins. Empty
	// means same-origin only.
	CORSAllowedOrigins []string
}

// NewRouter builds a router with the standard middleware chain applied.
func NewRouter(opts ...RouterOptions) chi.Router {
	o := RouterOptions{}
	if len(opts) > 0 {
		o = opts[0]
	}
	r := chi.NewRouter()
	r.Use(middleware.RequestID)
	r.Use(middleware.Recoverer)
	r.Use(LoggerMiddleware)
	r.Use(AuditActor)
	r.Use(SecurityHeaders())
	r.Use(CORS(o.CORSAllowedOrigins))
	r.Use(middleware.Timeout(30 * time.Second))
	return r
}

// NewServer returns an http.Server bound to the router. Timeouts and header
// caps are tuned for production: a short ReadHeaderTimeout guards against
// slowloris, and MaxHeaderBytes bounds memory per connection.
func NewServer(port int, router chi.Router) *http.Server {
	return &http.Server{
		Addr:              ":" + strconv.Itoa(port),
		Handler:           router,
		ReadTimeout:       15 * time.Second,
		ReadHeaderTimeout: 5 * time.Second,
		WriteTimeout:      30 * time.Second,
		IdleTimeout:       60 * time.Second,
		MaxHeaderBytes:    1 << 20, // 1 MiB
	}
}
