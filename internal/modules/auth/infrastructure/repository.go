...
// NewUserRepository builds a user repository bound to the shared pool, driver
// and app timezone.
...
// Save implements domain.UserRepository, inserting the user with all columns
...
// Save implements domain.RefreshTokenRepository, inserting the token with its
// hash, family and JTI.
...
// FindByHash implements domain.RefreshTokenRepository, returning (nil, nil)
...
// RevokeByID implements domain.RefreshTokenRepository, revoking the token if
...
// RevokeByIDIfActive implements domain.RefreshTokenRepository, reporting
...
// RevokeByUserID implements domain.RefreshTokenRepository, revoking every
...
// RevokeFamily implements domain.RefreshTokenRepository, revoking every
...
func (r *RefreshTokenRepository) RevokeFamily(ctx context.Context, familyID string) error {
	now := r.now()
	_, err := r.db.ExecContext(ctx, r.q(`UPDATE refresh_tokens SET revoked_at = ?, updated_at = ? WHERE family_id = ? AND revoked_at IS NULL`), now, now, familyID)
	return err
}

// JtisByFamily implements domain.RefreshTokenRepository, returning the
...
// JtisByUser implements domain.RefreshTokenRepository, returning every
...
// ListActiveFamilies implements domain.RefreshTokenRepository, returning the
...
// the shared pool, driver and app timezone.
...
// Save implements domain.VerificationCodeRepository, inserting the code with
...
// FindLatestActive implements domain.VerificationCodeRepository, returning the
...
// FindActiveByHash implements domain.VerificationCodeRepository, returning the
...
// Consume implements domain.VerificationCodeRepository. It only succeeds when
...
func (r *VerificationCodeRepository) Consume(ctx context.Context, id string) (bool, error) {
	now := r.now()
	res, err := r.db.ExecContext(ctx, r.q(`UPDATE verification_codes SET consumed_at = ?, updated_at = ? WHERE id = ? AND consumed_at IS NULL`), now, now, id)
	if err != nil {
		return false, err
	}
	n, err := res.RowsAffected()
...
// IncrementAttempts implements domain.VerificationCodeRepository. The counter
...
func (r *VerificationCodeRepository) IncrementAttempts(ctx context.Context, id string, maxAttempts int) error {
	now := r.now()
	res, err := r.db.ExecContext(ctx, r.q(`UPDATE verification_codes SET attempts = attempts + 1, updated_at = ? WHERE id = ? AND attempts < ?`), now, id, maxAttempts)
	if err != nil {
		return err
	}
	n, err := res.RowsAffected()
...
// InvalidateByUser implements domain.VerificationCodeRepository, consuming
...
// InvalidateByUserChannel implements domain.VerificationCodeRepository,
