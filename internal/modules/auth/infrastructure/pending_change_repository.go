...
// to the read/write pools, driver and app timezone.
func NewPendingContactChangeRepository(readDB, writeDB *sql.DB, driver string, loc *time.Location) *PendingContactChangeRepository {
	return &PendingContactChangeRepository{base{readDB: readDB, writeDB: writeDB, driver: driver, loc: loc}}
}

const pendingChangeColumns = `id, user_id, channel, old_value, new_value, status, applied_at, created_at, updated_at`

// Save implements domain.PendingContactChangeWriteRepository, inserting the pending
...
// FindPendingByNewValue implements domain.PendingContactChangeReadRepository,
...
// MarkApplied implements domain.PendingContactChangeWriteRepository, flipping a
