...
func testRouter(roles *fakeRoleRepo, perms *fakePermissionRepo) http.Handler {
	deps := Deps{
		CreateRole:          command.NewCreateRole(roles, nil),
		CreatePermission:    command.NewCreatePermission(perms, nil),
		UpdateRole:          command.NewUpdateRole(roles, nil, nil),
		DeleteRole:          command.NewDeleteRole(roles, nil, nil),
		UpdatePermission:    command.NewUpdatePermission(perms, nil, nil),
		DeletePermission:    command.NewDeletePermission(perms, nil, nil),
		AssignRole:          command.NewAssignRole(roles, nil, nil, nil),
		RevokeRole:          command.NewRevokeRole(roles, nil, nil, nil),
		GrantPermission:     command.NewGrantPermission(perms, nil, nil, nil),
		RevokePermission:    command.NewRevokePermission(perms, nil, nil, nil),
		SyncRolePermissions: command.NewSyncRolePermissions(roles, perms, nil, nil),
		GetRole:             query.NewGetRole(roles),
		ListRoles:           query.NewListRoles(roles),
		ListPermissions:     query.NewListPermissions(perms),
