...
// base carries the read/write pools and driver used by every repository type.
// readDB is the read replica (falls back to writeDB when nil).
type base struct {
	readDB  *sql.DB
	writeDB *sql.DB
	driver  string
...
func (b base) scanStrings(ctx context.Context, query string, args ...any) ([]string, error) {
	rows, err := b.r().QueryContext(ctx, b.q(query), args...)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func NewRoleRepository(readDB, writeDB *sql.DB, driver string) *RoleRepository {
	return &RoleRepository{base{readDB: readDB, writeDB: writeDB, driver: driver}}
}

// Save implements domain.RoleWriteRepository. It inserts the role row and returns
// an error if a role with the same id already exists.
func (r *RoleRepository) Save(ctx context.Context, role *domain.Role) error {
	_, err := r.w().ExecContext(ctx, r.q(`INSERT INTO roles (id, code, name) VALUES (?, ?, ?)`), role.ID, role.Code, role.Name)
	return err
}

// FindByCode implements domain.RoleReadRepository. It returns the role with code,
// or nil when no such role exists.
func (r *RoleRepository) FindByCode(ctx context.Context, code string) (*domain.Role, error) {
	row := r.r().QueryRowContext(ctx, r.q(`SELECT id, code, name FROM roles WHERE code = ?`), code)
	return scanRole(row)
}

// List implements domain.RoleReadRepository. It returns all roles ordered by
// creation time ascending.
func (r *RoleRepository) List(ctx context.Context) ([]*domain.Role, error) {
	rows, err := r.r().QueryContext(ctx, r.q(`SELECT id, code, name FROM roles ORDER BY created_at ASC`))
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
// Delete implements domain.RoleWriteRepository. Links are removed by ON DELETE
// CASCADE.
func (r *RoleRepository) Delete(ctx context.Context, id string) error {
	_, err := r.w().ExecContext(ctx, r.q(`DELETE FROM roles WHERE id = ?`), id)
	return err
}

// UpdateName implements domain.RoleWriteRepository. It renames the role's display
...
func (r *RoleRepository) UpdateName(ctx context.Context, id, name string) error {
	_, err := r.w().ExecContext(ctx, r.q(`UPDATE roles SET name = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`), name, id)
	return err
}

// SetPermissions implements domain.RoleWriteRepository. It deletes the role's
...
// PermissionsFor implements domain.RoleReadRepository. It returns the permission
...
// PermissionsForAll implements domain.RoleReadRepository. It loads every role's
...
func NewPermissionRepository(readDB, writeDB *sql.DB, driver string) *PermissionRepository {
	return &PermissionRepository{base{readDB: readDB, writeDB: writeDB, driver: driver}}
}

// Save implements domain.PermissionWriteRepository. It inserts the permission row
...
func (r *PermissionRepository) Save(ctx context.Context, p *domain.Permission) error {
	_, err := r.w().ExecContext(ctx, r.q(`INSERT INTO permissions (id, code, group_name, name) VALUES (?, ?, ?, ?)`), p.ID, p.Code, p.Group, p.Name)
	return err
}

// FindByCode implements domain.PermissionReadRepository. It returns the permission
...
func (r *PermissionRepository) FindByCode(ctx context.Context, code string) (*domain.Permission, error) {
	row := r.r().QueryRowContext(ctx, r.q(`SELECT id, code, group_name, name FROM permissions WHERE code = ?`), code)
...
// List implements domain.PermissionReadRepository. Results are ordered by group
...
func (r *PermissionRepository) List(ctx context.Context) ([]*domain.Permission, error) {
	rows, err := r.r().QueryContext(ctx, r.q(`SELECT id, code, group_name, name FROM permissions ORDER BY group_name ASC, code ASC`))
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
// Delete implements domain.PermissionWriteRepository. Links are removed by ON
// DELETE CASCADE.
func (r *PermissionRepository) Delete(ctx context.Context, id string) error {
	_, err := r.w().ExecContext(ctx, r.q(`DELETE FROM permissions WHERE id = ?`), id)
	return err
}

// Update implements domain.PermissionWriteRepository. It renames the display group
...
func (r *PermissionRepository) Update(ctx context.Context, id, group, name string) error {
	_, err := r.w().ExecContext(ctx, r.q(`UPDATE permissions SET group_name = ?, name = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`), group, name, id)
	return err
}
...
// placeholder dialect (mysql or postgres) used for rebinding queries.
func NewUserAccessRepository(readDB, writeDB *sql.DB, driver string) *UserAccessRepository {
	return &UserAccessRepository{base{readDB: readDB, writeDB: writeDB, driver: driver}}
}

// AssignRole implements domain.UserAccessWriteRepository. It inserts the link using
...
func (r *UserAccessRepository) AssignRole(ctx context.Context, userID, roleID string) error {
	_, err := r.w().ExecContext(ctx, r.q(r.insertIgnore("user_roles", "user_id, role_id", "?, ?")), userID, roleID)
	return err
}

// RevokeRole implements domain.UserAccessWriteRepository. It removes the link if it
// exists and does not error otherwise.
func (r *UserAccessRepository) RevokeRole(ctx context.Context, userID, roleID string) error {
	_, err := r.w().ExecContext(ctx, r.q(`DELETE FROM user_roles WHERE user_id = ? AND role_id = ?`), userID, roleID)
	return err
}

// GrantPermission implements domain.UserAccessWriteRepository. It inserts the link
...
func (r *UserAccessRepository) GrantPermission(ctx context.Context, userID, permissionID string) error {
	_, err := r.w().ExecContext(ctx, r.q(r.insertIgnore("user_permissions", "user_id, permission_id", "?, ?")), userID, permissionID)
	return err
}

// RevokePermission implements domain.UserAccessWriteRepository. It removes the link
...
func (r *UserAccessRepository) RevokePermission(ctx context.Context, userID, permissionID string) error {
	_, err := r.w().ExecContext(ctx, r.q(`DELETE FROM user_permissions WHERE user_id = ? AND permission_id = ?`), userID, permissionID)
	return err
}

// Roles implements domain.UserAccessReadRepository. It returns the role codes of
...
// CountUsersWithRole implements domain.UserAccessReadRepository. It counts the
...
func (r *UserAccessRepository) CountUsersWithRole(ctx context.Context, roleID string) (int, error) {
	var n int
	err := r.r().QueryRowContext(ctx, r.q(`SELECT COUNT(*) FROM user_roles WHERE role_id = ?`), roleID).Scan(&n)
	return n, err
}

// DirectPermissions implements domain.UserAccessReadRepository. It returns
// permission codes.
...
// RolePermissionCodes implements domain.UserAccessReadRepository. It returns the
