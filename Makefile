...
check: build vet test fmt
	...
	@! rg -n "internal/modules/(rbac)/(domain|application|infrastructure|adapter)" internal/modules/auth || true
	@! rg -n "internal/modules/(auth)/(domain|application|infrastructure|adapter)" internal/modules/rbac || true
